const DocTest = (function($, $C){const $H=$C.simple;
	...
	function argumentList(defs, showTitle){
		if(showTitle==null) showTitle = true;
		return $H.div({'class':'argumentList'},
			showTitle?$H.div('Аргументы:'):null,
			$H.ol(
	...
	function testedSample(val, expectation){
		function formatVal(val){
			return val==null?'null'
				...
				:val instanceof Set?['new Set([', Array.from(val),'])'].join('')
				:val instanceof RegExp?['/', val.source, '/'].join('')
				:JSON.stringify(val);
		}
		...
		return (itm, nm)=>{
			...
			// const equal = JSON.stringify(res)==JSON.stringify(expectation);
			const equal = formatVal(res)==formatVal(expectation);
	...
	function attention(content){
		return $H.div({'class':'attention'}, content);
	}

	function instanceObject(title, constructorFunction){
		const inst = constructorFunction();
		return function(...content){
			// console.log('instanceObject: %o', inst);
			const {markup,apply,p,div} = $H;
			return markup(
				//p({'class':'undefinedItem'}, 'INSTANCE OBJECT (not implemented)'),
				p(title),
				apply(content, el=>{
					// console.log(el);
					return typeof(el)=='function'?el(inst):el;
				})
			);
		}
	}
